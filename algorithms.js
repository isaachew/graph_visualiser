...
var algorithms={
    dijkstra:{
        parameters:[{type:"vertex",name:"starting vertex"},{type:"vertex",name:"ending vertex"}],
    ...
    dfs:{
        parameters:[{type:"vertex",name:"starting vertex"}],
        run(vert){
            var st=[]
            var steps=[]
            var adj=curGraph.vertices.map(a=>[])
    ...
    nn:{
        ...
        run(vert){//Nearest neighbour algorithm
            var distmat=[]
            var steps=[]
            var adj=curGraph.vertices.map(a=>[])
            for(var i=0;i<curGraph.edges.length;i++){
    ...
    cpa:{
        ...
        run(){//assumes all edges are directed
            var adj=curGraph.vertices.map(a=>[])
