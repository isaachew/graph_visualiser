...
function dijkstra(vert,dest){
    ...
    while(pq.length){
        ...
        if(cur[2]!=-1)steps.push([{type:"edge",index:cur[2],style:dijkstraStyles.tree}])//shortest path tree
        clast=cur[0]
        last[cur[0]]=cur[2]
        dists[cur[0]]=cur[1]
        ...
        for(var i=0;i<adj[cur[0]].length;i++){
            ...
            if(cedge.v1==cur[0]&&dists[cedge.v2]==-1){
                steps.push([{type:"edge",index:eind,style:dijkstraStyles.considered}])
    ...
    while(clast!=vert){
        steps.push([{type:"edge",index:last[clast],style:dijkstraStyles.path}])
...
function prim(vert){
    ...
    while(pq.length){
        ...
        var cur=pq.pop()
        if(visited[cur[0]]){
            if(cur[2]!=-1)steps.push([{type:"edge",index:cur[2],style:primStyles.unused}])
            continue
        }
        visited[cur[0]]=true
        if(cur[2]!=-1)steps.push([{type:"edge",index:cur[2],style:primStyles.used}])
        clast=cur[0]
        steps.push([{type:"vertex",index:cur[0],style:{colour:"red"}}])
        for(var i=0;i<adj[cur[0]].length;i++){
            var eind=adj[cur[0]][i]
            ...
            if(cedge.v1==cur[0]&&!visited[cedge.v2]){
                steps.push([{type:"edge",index:eind,style:primStyles.considered}])
...
function kruskal(){
    ...
    for(var i=0;i<curGraph.edges.length;i++){
        ...
        steps.push([{type:"edge",index:eind,style:kruskalStyles.considered}])
        if(merge(curEdge.v1,curEdge.v2)){
            steps.push([{type:"edge",index:eind,style:kruskalStyles.used}])
        }else{
            steps.push([{type:"edge",index:eind,style:kruskalStyles.unused}])
        }
    }
    return steps
}
